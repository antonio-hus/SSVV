...
const descriptors: Array<{descriptor: FtDescriptor; outFile: string}> = [
    {
        ...
        descriptor: {
            ...
            tcRows: [
                ...
                tc('TC-07', 'Rejects notes over the schema limit and does not call create.', 'Select member, set valid date, and set Notes to 1025 characters.', 'userEvent.click(Create Session)', 'Validation failed alert and Notes max-length field error visible; createWorkoutSession not called.'),
                tc('TC-08', 'Rejects an invalid exercise row and does not call create.', 'Select member, set valid date, leave Exercise empty, and set sets/reps/weight above their upper bounds.', 'userEvent.click(Create Session)', 'Validation failed plus Exercise, Sets, Reps, and Weight field errors are visible; createWorkoutSession not called.'),
                tc('TC-09', 'Adds a second exercise row and enables removal.', 'render form.', 'userEvent.click(Add Exercise)', 'Member select plus two exercise selects exist; two Remove buttons exist and the first is enabled.'),
                tc('TC-10', 'Removes the second exercise row and disables the remaining Remove button.', 'Add a second exercise row.', 'Click the second Remove button.', 'Member select plus one exercise select remain; the sole Remove button is disabled.'),
                tc('TC-11', 'Submits edited metadata and row values, then navigates to member detail.', 'createWorkoutSession resolves success; fill member/date, edit duration, notes, exercise, sets, reps, and weight.', 'userEvent.click(Create Session)', 'createWorkoutSession called with trimmed notes and numeric row values; router.push /admin/members/mem-1.'),
                tc('TC-12', 'Submits an added exercise row as a second parsed row.', 'createWorkoutSession resolves success; fill valid session, add second row, select exercise.', 'userEvent.click(Create Session)', 'createWorkoutSession receives two parsed exercise rows; router.push /admin/members/mem-1.'),
                tc('TC-13', 'Shows server error and does not navigate when create fails.', 'createWorkoutSession resolves failed result.', 'userEvent.click(Create Session)', 'Server error visible; Create Session enabled; create called once; router.push not called.'),
                tc('TC-14', 'Displays returned server field errors for editable metadata.', 'createWorkoutSession resolves failed result with memberId/date/duration errors.', 'userEvent.click(Create Session)', 'Validation failed alert and returned member/date/duration errors are visible; router.push not called.'),
                tc('TC-15', 'Disables submit control while create is pending.', 'createWorkoutSession returns an unresolved promise after valid input.', 'userEvent.click(Create Session)', 'Creating... button is disabled and Create Session text is absent until the promise resolves.'),
            ],
        },
    },
    {
        ...
        descriptor: {
            ...
            tcRows: [
                ...
                tc('TC-06', 'Rejects notes over the schema limit and does not call update.', 'Type 1025 characters into Notes.', 'userEvent.click(Save Changes)', 'Validation failed alert and Notes max-length field error visible; update action not called.'),
                tc('TC-07', 'Rejects an invalid exercise row and does not call update.', 'Clear Exercise and set sets/reps/weight above their upper bounds.', 'userEvent.click(Save Changes)', 'Validation failed plus Exercise, Sets, Reps, and Weight field errors are visible; update action not called.'),
                tc('TC-08', 'Adds a second exercise row and enables removal.', 'render form.', 'userEvent.click(Add Exercise)', 'Two exercise selects and two Remove buttons exist; the first Remove is enabled.'),
                tc('TC-09', 'Removes the second exercise row and disables the remaining Remove button.', 'Add a second row.', 'Click the second Remove button.', 'One exercise select remains and the sole Remove button is disabled.'),
                tc('TC-10', 'Calls update action with edited metadata and parsed existing row, then refreshes.', 'update action resolves success; edit duration, notes, exercise, sets, reps, and weight.', 'userEvent.click(Save Changes)', 'Success alert visible; update action called with trimmed notes and numeric row values; refresh called once; push not called.'),
                tc('TC-11', 'Submits a newly added row without an existing row ID.', 'update action resolves success; add a row and select an exercise.', 'userEvent.click(Save Changes)', 'update action receives the original row plus a new parsed row with id undefined.'),
                tc('TC-12', 'Shows server error and does not refresh when update fails.', 'update action resolves failed result.', 'userEvent.click(Save Changes)', 'Server error visible; Save Changes enabled; refresh and push not called.'),
                tc('TC-13', 'Displays returned server field errors for editable metadata.', 'update action resolves failed result with date and duration errors.', 'userEvent.click(Save Changes)', 'Validation failed alert plus returned date and duration errors are visible; refresh not called.'),
                tc('TC-14', 'Deletes session, refreshes, and navigates to member detail.', 'delete action resolves success.', 'userEvent.click(Delete Session)', 'delete called with session-1; refresh called; router.push /admin/members/mem-1.'),
                tc('TC-15', 'Shows delete error and does not navigate or refresh when delete fails.', 'delete action resolves failed result.', 'userEvent.click(Delete Session)', 'Server error visible; delete called once; router.push and refresh not called.'),
                tc('TC-16', 'Disables save and delete controls while update is pending.', 'update action returns an unresolved promise.', 'userEvent.click(Save Changes)', 'Saving button is disabled, Delete Session is disabled, and Save Changes text is absent until the promise resolves.'),
            ],
        },
    },
    {
